...
void Motor_Forward_R(void)
{
	// Enable the clock to Port C
	SYSCTL->RCGCGPIO |= 0x04;
	
	// Set PC4, PC5 as output GPIO pins
	GPIOC->DIR |= 0x30;
	
	// Configure PC4, PC5 to function as GPIO pins
	GPIOC->AFSEL &= ~0x30;
	
	// Enable digital functionality for PC4, PC5
	GPIOC->DEN |= 0x30;
	
	// Initialize PC5->1 and PC4->0
	GPIOC->DATA = GPIOC->DATA |= 0x20;
	GPIOC->DATA = GPIOC->DATA &= ~0x10;
}

void Motor_Reverse_R(void)
{
	// Enable the clock to Port C
	SYSCTL->RCGCGPIO |= 0x04;
	...
	// Enable digital functionality for PC4, and PC5
	GPIOC->DEN |= 0x30;
	
	// Initialize PC5->0 and PC4->1
	 GPIOC->DATA = GPIOC->DATA |= 0x10;
	 GPIOC->DATA = GPIOC->DATA &= ~0x20;
}

void Motor_Reverse_L(void)
{
	// Enable the clock to Port C
	SYSCTL->RCGCGPIO |= 0x04;
	
	// Set PC6, PC7 as output GPIO pins
	GPIOC->DIR |= 0xC0;
	
	// Configure PC6, PC7 to function as GPIO pins
	GPIOC->AFSEL &= ~0xC0;
	
	// Enable digital functionality for PC6, and PC7
	GPIOC->DEN |= 0xC0;
	
	// Initialize PC6->0 and PC7->1
	 GPIOC->DATA = GPIOC->DATA |= 0x80;
	 GPIOC->DATA = GPIOC->DATA &= ~0x40;
}

void Motor_Forward_L(void)
{
	// Enable the clock to Port C
	SYSCTL->RCGCGPIO |= 0x04;
