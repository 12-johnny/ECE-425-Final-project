...
{	
 if (duty_cycle >= period_constant)return; 
	...
	// Configure the PE2 pin to operate as a Module 1 PWM6 pin (M1PWM6)
	...
	// Disable the Module 1 PWM Generator 3 block (PWMI_3) before
	...
	PWM1->_3_CTL &= ~0x02;
		...
		// Configure the counter for the PWMI_3 block to
	// use Count-Down mode by clearing the MODE bit (Bit 1)
	// in the PWM3CTL register. The counter will count from the load value
	// to 0, and then wrap back to the load value
	PWM1->_3_CTL &= ~0x02;
	...
	// Enable the PWMI_3 block after configuration by setting the
